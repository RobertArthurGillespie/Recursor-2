...
public static class StableMasteryModelTrainer
{
    ...
    public static void Train(string csvPath, string modelOutputPath)
    {
        ...
        Console.WriteLine($"[StableMasteryModelTrainer] Loading data from: {csvPath}");
        ...
        Console.WriteLine($"[StableMasteryModelTrainer] Parsed rows: {rows.Count}");

        if (rows.Count == 0)
            throw new InvalidOperationException("No rows were parsed from the CSV.");

        int trueCount  = rows.Count(r => r.LabelStableMastery);
        int falseCount = rows.Count(r => !r.LabelStableMastery);

        Console.WriteLine($"[StableMasteryModelTrainer] LabelStableMastery TRUE rows:  {trueCount}");
        Console.WriteLine($"[StableMasteryModelTrainer] LabelStableMastery FALSE rows: {falseCount}");

        if (trueCount == 0 || falseCount == 0)
            throw new InvalidOperationException(
                $"Dataset must contain both TRUE and FALSE LabelStableMastery values. TRUE={trueCount}, FALSE={falseCount}");
        ...
        Console.WriteLine($"[StableMasteryModelTrainer] Train rows: {trainRows}");
        Console.WriteLine($"[StableMasteryModelTrainer] Test rows:  {testRows}");
        ...
        var pipeline =
            ...
            .Append(mlContext.BinaryClassification.Trainers.SdcaLogisticRegression(
                labelColumnName:   nameof(BehaviorStateTrainingExample.LabelStableMastery),
                featureColumnName: "Features"));

        Console.WriteLine("[StableMasteryModelTrainer] Training...");
        var model = pipeline.Fit(split.TrainSet);

        Console.WriteLine("[StableMasteryModelTrainer] Evaluating on test set...");
        ...
        Console.WriteLine($"[StableMasteryModelTrainer] Model saved to: {modelOutputPath}");
    }
}
